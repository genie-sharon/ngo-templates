...
for (const [slug, dir] of Object.entries(slugToDir)) {
  ...
  console.log(
    `Template "${slug}" (dir: ${dir}): ${templateImageMap[slug].length} images available`,
  );
}
...
function fixTemplate(filePath) {
  ...
  content = content.replace(newsImgPattern, (_match) => {
  ...
  const aboutImgPattern =
    /image:\s*\{[^}]*src:\s*'\/images\/[^/]+\/(?:gallery\d*|hero)\.jpg'[^}]*\}/g;
  ...
  content = content.replace(
    ...
    (match) => {
      ...
      const personImg =
        images.find((f) => f.includes('person') || f.includes('about')) || images[0];
  ...
  let genericIdx = 0;
  content = content.replace(genericImgPattern, (_match) => {
  ...
  function badgeToSvg(alt, name) {
    ...
    if (
      lower.includes('un ecosoc') ||
      lower.includes('united nations') ||
      lower.includes('un consultative')
    )
      svgFile = 'un-ecosoc.svg';
    else if (lower.includes('charity navigator')) svgFile = 'charity-navigator.svg';
    else if (lower.includes('guidestar') || lower.includes('candid') || lower.includes('gold'))
      svgFile = 'gold-transparency.svg';
...
const templateFiles = readdirSync(templatesDir).filter(
  (f) => f.endsWith('.ts') && !['types.ts', 'registry.ts', 'helpers.ts'].includes(f),
);
